...
function MembershipCard({ imageSrc, number, title, description, highlightColor }: MembershipCardProps) {
  return (
    <div className="relative h-[280px] md:h-[314px] bg-white rounded-[20px] md:rounded-[30px] shadow-lg p-4" style={{ boxShadow: '0px 5px 15px rgba(0, 0, 0, 0.25)' }}>
      <img src={imageSrc} alt={title} className="w-[120px] md:w-[146px] h-[60px] md:h-[74px] object-cover" />
      <div className="absolute bottom-8 md:bottom-10 left-4 right-4">
        <div className={`text-3xl md:text-[48px] font-aleo font-bold leading-tight md:leading-[58px] mb-2 md:mb-3`} style={{ color: highlightColor }}>
          {number}
        </div>
        <div className="text-xs md:text-[15px] font-manrope leading-relaxed md:leading-[19.5px]" dangerouslySetInnerHTML={{ __html: description }} />
      </div>
    </div>
  )
}
  ...
  return (
    <>
      <p className="mb-6 md:mb-8 text-lg md:text-xl font-normal leading-relaxed md:leading-[26px] text-[#9B2683]">
        The raiSE membership includes <span className="text-xl md:text-2xl font-bold">693</span> businesses across the different
        stages of their Social Enterprise journey.
      </p>

      {/* Four Cards Section - 2 per row */}
      <div className="mb-12 md:mb-16 grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        <MembershipCard 
          imageSrc="https://placehold.co/146x74"
          number="335"
